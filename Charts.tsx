...
export const PriceAndIndicatorCharts: React.FC<PriceChartProps> = ({ stockData, indicators, backtestResult, theme }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="p-4 bg-white dark:bg-slate-800 rounded-xl shadow-md border border-slate-200 dark:border-slate-700">
        <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-200 mb-4">Price and Trading Signals</h3>
        <ResponsiveContainer width="100%" height={400}>
          <ComposedChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" stroke={gridColor} />
            <XAxis dataKey="date" tick={{ fontSize: 12, fill: textColor }} tickFormatter={(d) => d.slice(0, 7)} />
            <YAxis yAxisId="left" orientation="left" tick={{ fontSize: 12, fill: textColor }} domain={['dataMin - 5', 'dataMax + 5']} tickFormatter={(p) => typeof p === 'number' ? `$${p.toFixed(0)}` : p} />
            <Tooltip
            ...
            <Area type="monotone" dataKey="price" name="Price" yAxisId="left" stroke="#3b82f6" fill="#3b82f6" fillOpacity={isDark ? 0.3 : 0.1} strokeWidth={2} />
            <Line type="monotone" dataKey="upperBB" name="Upper BB" yAxisId="left" stroke="#ef4444" dot={false} strokeWidth={1} strokeDasharray="5 5" />
            <Line type="monotone" dataKey="middleBB" name="Middle BB" yAxisId="left" stroke={isDark ? '#94a3b8' : '#64748b'} dot={false} strokeWidth={1} strokeDasharray="3 3" />
      ...
      <div className="p-4 bg-white dark:bg-slate-800 rounded-xl shadow-md border border-slate-200 dark:border-slate-700">
        <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-200 mb-4">Technical Indicators (MACD & RSI)</h3>
        <ResponsiveContainer width="100%" height={250}>
          <ComposedChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" stroke={gridColor} />
            <XAxis dataKey="date" tick={{ fontSize: 12, fill: textColor }} tickFormatter={(d) => d.slice(0, 7)} />
            <YAxis yAxisId="left" orientation="left" tick={{ fontSize: 12, fill: textColor }} />
            <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 12, fill: textColor }} domain={[0, 100]} />
            <Tooltip
              formatter={(value) => {
                  if (typeof value === 'number') {
                      return value.toFixed(2);
            ...
            <Bar yAxisId="left" dataKey="histogram" name="Histogram" barSize={5}>
              {chartData.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={entry.histogram && entry.histogram > 0 ? (isDark ? 'rgba(34, 197, 94, 0.6)' : 'rgba(34, 197, 94, 0.5)') : (isDark ? 'rgba(239, 68, 68, 0.6)' : 'rgba(239, 68, 68, 0.5)')} />
              ))}
            </Bar>
...
export const HeatmapChart: React.FC<HeatmapProps> = ({ optimizationResult, theme }) => {
    ...
    }, [optimizationResult, theme]);

    return (
        <div className="w-full h-full relative">
            <canvas ref={canvasRef} className="w-full h-full"></canvas>
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 transform flex justify-center py-2 gap-4">
                <div className="flex items-center gap-2"><div className="w-4 h-4 rounded" style={{ background: '#3d85c6' }}></div><span className="text-xs text-slate-600 dark:text-slate-300">Low</span></div>
                <div className="flex items-center gap-2"><div className="w-4 h-4 rounded" style={{ background: '#e69138' }}></div><span className="text-xs text-slate-600 dark:text-slate-300">Medium</span></div>
                <div className="flex items-center gap-2"><div className="w-4 h-4 rounded" style={{ background: '#8fce00' }}></div><span className="text-xs text-slate-600 dark:text-slate-300">Good</span></div>
                <div className="flex items-center gap-2"><div className="w-4 h-4 rounded" style={{ background: '#ff0000' }}></div><span className="text-xs text-slate-600 dark:text-slate-300">High</span></div>
            </div>
        </div>
    );
};